...
in
{
  ...
  home.file.".config/niri/config.kdl" = {
    text = ''
      ...
      spawn-at-startup "${daynightscreen}"

      // Environment variables - Enhanced for XWayland compatibility
      environment {
          // Wayland-specific
      ...
      binds {
          ...
          // Screenshots
          // Mod+Shift+S { spawn "${screenshot_select}"; }
          // Mod+Shift+A { spawn "${screenshot_full}"; }
  ...
  # Enhanced systemd user service for comprehensive Wayland/XWayland session variables
  systemd.user.services.niri-session-env = {
    Unit = {
